\
set
  ON_ERROR_STOP on \ connect brinco_de_princesa
SET
  ROLE brinco_owner;
...
CREATE TABLE IF NOT EXISTS usuarios_admin (
  ...
  papel VARCHAR(24) NOT NULL DEFAULT 'MANAGER' CHECK (papel IN ('OWNER', 'MANAGER', 'FULFILLMENT')),
  ativo BOOLEAN NOT NULL DEFAULT TRUE,
  tentativas_login_falhas SMALLINT NOT NULL DEFAULT 0 CHECK (tentativas_login_falhas >= 0),
...
CREATE TABLE IF NOT EXISTS categorias (
  ...
  criado_em TIMESTAMPTZ NOT NULL DEFAULT now(),
  atualizado_em TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT categorias_sem_auto_referencia CHECK (
    categoria_pai_id IS NULL
    OR categoria_pai_id <> id
  ),
...
CREATE TABLE IF NOT EXISTS produto_imagens (
  ...
  variante_id BIGINT REFERENCES produto_variantes(id) ON DELETE
  SET
    NULL,
    url TEXT NOT NULL,
    alt_text VARCHAR(180) NOT NULL,
    mime_type VARCHAR(80),
    largura_px INTEGER CHECK (
      largura_px IS NULL
      OR largura_px > 0
    ),
    altura_px INTEGER CHECK (
      altura_px IS NULL
      OR altura_px > 0
    ),
    ordem SMALLINT NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS cupons (
  ...
  desconto_maximo NUMERIC(12, 2) CHECK (
    desconto_maximo IS NULL
    OR desconto_maximo > 0
  ),
  limite_usos INTEGER CHECK (
    limite_usos IS NULL
    OR limite_usos > 0
  ),
  ...
  CONSTRAINT cupons_periodo_valido CHECK (
    expira_em IS NULL
    OR inicia_em IS NULL
    OR expira_em > inicia_em
  )
);
...
CREATE TABLE IF NOT EXISTS pedidos (
  ...
  cupom_id BIGINT REFERENCES cupons(id) ON DELETE
  SET
    NULL,
    status VARCHAR(24) NOT NULL DEFAULT 'PENDING_PAYMENT' CHECK (
      status IN (
        'PENDING_PAYMENT',
        'PAID',
        'IN_PRODUCTION',
...
CREATE TABLE IF NOT EXISTS pedido_itens (
  ...
  produto_id BIGINT REFERENCES produtos(id) ON DELETE
  SET
    NULL,
    variante_id BIGINT REFERENCES produto_variantes(id) ON DELETE
  SET
...
CREATE TABLE IF NOT EXISTS pagamentos (
  ...
  gateway_preference_id VARCHAR(160),
...
CREATE TABLE IF NOT EXISTS webhook_eventos (
  ...
  status_processamento VARCHAR(16) NOT NULL DEFAULT 'RECEIVED' CHECK (
    status_processamento IN ('RECEIVED', 'PROCESSED', 'IGNORED', 'FAILED')
  ),
...
CREATE TABLE IF NOT EXISTS audit_logs (
  id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  admin_id BIGINT REFERENCES usuarios_admin(id) ON DELETE
  SET
    NULL,
...
CREATE TABLE IF NOT EXISTS configuracoes (
  ...
  atualizado_por BIGINT REFERENCES usuarios_admin(id) ON DELETE
  SET
    NULL,
    atualizado_em TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS banners (
  ...
  criado_em TIMESTAMPTZ NOT NULL DEFAULT now(),
  atualizado_em TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT banners_periodo_valido CHECK (
    termina_em IS NULL
    OR inicia_em IS NULL
...
CREATE TABLE IF NOT EXISTS idempotency_keys (
  ...
  status_code SMALLINT CHECK (
    status_code IS NULL
    OR status_code BETWEEN 100
    AND 599
  ),
  response_body JSONB,
  recurso_id VARCHAR(100),
...
CREATE
OR REPLACE FUNCTION set_atualizado_em() RETURNS TRIGGER LANGUAGE plpgsql AS $ $ BEGIN NEW.atualizado_em = now();
...
BEGIN FOREACH tabela IN ARRAY ARRAY [
  ...
  'DROP TRIGGER IF EXISTS set_%I_atualizado_em ON %I',
  tabela,
  tabela
);
...
ALTER DEFAULT PRIVILEGES FOR ROLE brinco_owner IN SCHEMA app REVOKE ALL PRIVILEGES ON TABLES
FROM
  brinco_app;

ALTER DEFAULT PRIVILEGES FOR ROLE brinco_owner IN SCHEMA app REVOKE ALL PRIVILEGES ON SEQUENCES
FROM
  brinco_app;

ALTER DEFAULT PRIVILEGES FOR ROLE brinco_owner IN SCHEMA app REVOKE ALL PRIVILEGES ON FUNCTIONS
FROM
  brinco_app;

ALTER DEFAULT PRIVILEGES FOR ROLE brinco_owner IN SCHEMA app REVOKE EXECUTE ON FUNCTIONS
FROM
  PUBLIC;
...
SELECT
  ...
  app.colecoes,
  app.produtos,
  app.produto_variantes,
  app.produto_imagens,
  app.produto_colecoes TO brinco_app;
