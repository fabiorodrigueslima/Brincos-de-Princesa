...
vi.mock('../components/common/Icons.jsx', () => ({ MenuIcon: () => null, ShoppingBagIcon: () => null, XIcon: () => null }))
vi.mock('../components/common/ScrollToTop.jsx', () => ({ ScrollToTop: () => null }))
...
describe('responsive site header', () => {
  it('renders one persistent cart action beside the mobile menu', () => {
    ...
    expect(markup).toContain('aria-label="Carrinho, 0 itens"')
  ...
  it('announces and displays the current shared cart count', () => {
    ...
    expect(markup).toContain('aria-label="Carrinho, 3 itens"')
  ...
  it('keeps the cart visible in mobile CSS with touch-sized actions', () => {
    ...
    expect(css).toMatch(/\.store-shortcut\s*\{[^}]*min-width\s*:\s*44px[^}]*min-height\s*:\s*44px/s)
    expect(css).toMatch(/\.menu-toggle\s*\{[^}]*width\s*:\s*44px[^}]*height\s*:\s*44px/s)
  })
})
