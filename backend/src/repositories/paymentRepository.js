    ...
    async processWebhook(event) {
      return runTransaction(async (client) => {
        ...
        const payment = await client.query({
          text: `SELECT pa.id,pa.pedido_id,pa.status,pa.valor,pa.moeda,p.status pedido_status,p.codigo_publico FROM app.pagamentos pa JOIN app.pedidos p ON p.id=pa.pedido_id WHERE pa.provedor=$1 AND p.codigo_publico=$2 FOR UPDATE OF pa,p`,
          values: [event.provider, event.reference],
        });
        if (!payment.rowCount) {
          await finishEvent(
            client,
        ...
        if (
          ...
          await releaseOrder(
            client,
            state.pedido_id,
            `Pagamento ${event.status.toLowerCase()}`,
          );
        }
        if (event.status === "REFUNDED" && state.status !== "REFUNDED") {
          ...
          const changed = await client.query({ text: `UPDATE app.pedidos SET status='REFUNDED' WHERE id=$1 AND status <> 'REFUNDED' RETURNING status`, values: [state.pedido_id] });
...
async function releaseOrder(client, orderId, reason) {
  const reservations = await client.query({
    text: `SELECT id,variante_id,quantidade FROM app.reservas_estoque WHERE pedido_id=$1 AND status='ACTIVE' FOR UPDATE`,
    values: [orderId],
  });
  for (const reservation of reservations.rows) {
    const stock = await client.query({
    ...
    await client.query({
      text: `UPDATE app.reservas_estoque SET status='RELEASED' WHERE id=$1`,
      values: [reservation.id],
    });
    await client.query({
      text: `INSERT INTO app.movimentos_estoque(variante_id,pedido_id,tipo,quantidade,saldo_apos,motivo) VALUES($1,$2,'RELEASE',$3,$4,$5)`,
      values: [
        reservation.variante_id,
        orderId,
        -reservation.quantidade,
  ...
  const changed = await client.query({
    text: `UPDATE app.pedidos SET status='CANCELLED',cancelado_em=now() WHERE id=$1 AND status='PENDING_PAYMENT' RETURNING id`,
    values: [orderId],
  });
  if (changed.rowCount)
    await client.query({
      text: `INSERT INTO app.pedido_status_historico(pedido_id,status_anterior,status_novo,motivo) VALUES($1,'PENDING_PAYMENT','CANCELLED',$2)`,
