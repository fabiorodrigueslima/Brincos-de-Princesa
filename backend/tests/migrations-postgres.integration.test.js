...
describe('PostgreSQL migrations', () => {
  beforeAll(async () => {
    if (!testAdminDatabaseUrl || !databaseName.endsWith('_test')) {
      throw new Error('Migrations de integração exigem o banco protegido com sufixo _test')
    }
  ...
  it('rejects a changed checksum for an applied migration', async () => {
    const original = await readFile(resolve(migrationsDirectory, '001_baseline.sql'), 'utf8')
    const directory = await temporaryMigrations({
      '001_baseline.sql': `${original}\n-- alteração proibida`,
    })

    await expect(runMigrations({ client, directory })).rejects.toThrow('foi alterada depois de aplicada')
  })
  ...
  it('rolls back every statement and registration when a migration fails', async () => {
    ...
    await expect(runMigrations({ client, directory })).rejects.toThrow('Falha na migration')
