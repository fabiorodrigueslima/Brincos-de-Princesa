...
describe('admin API security', () => {
  it('rejects every protected resource without a session', async () => { for(const path of ['/dashboard','/products','/stock','/orders','/courses','/settings'])expect((await request(app).get(`/api/v1/admin${path}`)).status).toBe(401) })
