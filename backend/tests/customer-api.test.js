...
describe('customer API security boundary',()=>{
  ...
  it('protects profile, orders and addresses without a session',async()=>{for(const path of ['/api/v1/customers/me','/api/v1/customers/orders'])expect((await request(app).get(path)).status).toBe(401);expect((await request(app).post('/api/v1/customers/addresses').send({})).status).toBe(401)})
